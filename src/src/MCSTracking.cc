...
MCSTracking::MCSTracking(int tracking_mode, uint16_t target_rnti, bool en_debug, int sniffer_mode, int api_mode, std::atomic<float> &est_cfo):
                                        ...
                                        float ta)
{
    ...
    if (iter!= tracking_database_ul_mode.end())
    {
        ...
        if (success) {iter->second.nof_success_mgs++;}
...
void MCSTracking::update_database_dl(){
    ...
    for (iter = tracking_database_dl_mode.begin(); iter != tracking_database_dl_mode.end(); iter++){
        ...
        if (cur_interval > interval || wrong_detect || iter->second.nof_active == 0){
            ...
            if (cur_success_rate < (float)0.15 && iter->second.mcs_table != DL_SNIFFER_UNKNOWN_TABLE){ //if success rate lower than 15%, reset mcs table
                iter->second.mcs_table = DL_SNIFFER_UNKNOWN_TABLE;
            }
        }
    }
            ...
            for (int cw = 0; cw < SRSRAN_MAX_CODEWORDS; cw++){
                if (tb_en[cw]){
                    if (statistic_grant->tb[cw].mcs_idx == mcs_idx){
                        ...
                        if (success[cw]) {iter->second.mcs_sc[mcs_idx]++;}
                        if (!success[cw] && transmission_type[cw] == DL_SNIFFER_DECODED) {iter->second.mcs_sc[mcs_idx]++;}
                    }
